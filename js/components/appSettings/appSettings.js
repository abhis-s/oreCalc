...
import { importUserData } from '../../utils/cloudSaveHandler.js';
...
export function initializeAppSettings() {
    ...
    if (importUserDataBtn && importUserIdInput) {
        importUserDataBtn.addEventListener('click', async () => {
            ...
            if (manualImportId) {
                // If user has manually entered an ID, use that directly
            ...
            try {
                ...
                if (clipboardText) {
                    importUserData(clipboardText.trim());
                } else {
                    // No text in clipboard, or permission denied for readText
                    // Fallback to showing input field
                    importUserIdInput.style.display = 'block'; // Show the input field
                    importUserDataBtn.querySelector('.animated-btn-text').textContent = 'Import'; // Change button text
                ...
                // Permission denied or other error reading clipboard
